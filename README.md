...
GitHub Actions seeds Key Vault (`jwt-signing-key`, `sql-admin-password`) from existing GitHub secrets. The Container App reads the JWT signing key through a Key Vault reference and the user-assigned identity (Key Vault Secrets User). On Azure, the API connects to SQL with **Microsoft Entra ID and that same identity** — no SQL password in the connection string. The deploy workflow creates the database user from the identity's **client ID** (SID), the same GUID as `User Id` in the connection string, so the SQL server does not need Directory Readers. Using the object ID here causes `18456` (`<token-identified principal>`). Local and Docker still use a SQL login (LocalDB / `sa`).
...
The browser calls the Container Apps API by HTTPS (CORS). Local and Docker still use relative `/api` URLs and do not use Key Vault.
